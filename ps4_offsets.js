...
// 11.52 shares the 11.50 WebKit/kernel family and patch blob. Keep this as an
// explicit alias so firmware detection never silently falls through to a
// neighboring table.
PS4["11.52"] = Object.assign({}, PS4["11.50"], {
    alias_of: "11.50",
    fw_status: "state=UNTESTED-on-hardware shares=11.50 "
        + "kpatch=1150.bin bug=poops",
    kpatch: "1150.bin",
});
...
// Experimental 13.x profiles requested for R&D. They intentionally route to
// the complete chain instead of being blocked, but reuse the 13.00 WebKit and
// kernel table until firmware-specific module/kernel dumps are available.
// Exact keys prevent an unrelated firmware from silently selecting this row.
for (const firmware of ["13.02", "13.04", "13.50", "13.52"]) {
    PS4[firmware] = Object.assign({}, PS4["13.00"], {
        alias_of: "13.00",
        fw_status: "state=R&D-UNTESTED-on-hardware shares=13.00 "
            + "webkit=assumed-identical kernel_rvas=assumed-identical "
